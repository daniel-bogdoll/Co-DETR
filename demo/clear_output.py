...
def move_best_model(config_file, dataset_name):
    ...
    # Find best model
    best_models = [f for f in os.listdir(output_folder_codetr) if "best_bbox" in f and f.endswith(".pth")]
    if not best_models:
        print("Cannot find a checkpoint file with 'best_bbox' found.")
    else:
        if len(best_models) > 1:
            print(f"Found {len(best_models)} checkpoint files. Selecting {best_models[0]}.")
        ...
        new_checkpoint_path = os.path.join(output_dir, f"{param_config_name}_{dataset_name}.pth")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Clear output folder and process dataset.")
